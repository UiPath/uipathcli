...
func (c DataServiceCommand) getEntityDefinition(operationId string) (*parser.Definition, error) {
	baseUri, err := url.Parse("https://cloud.uipath.com/{organization}/{tenant}/dataservice_")
	if err != nil {
		return nil, err
	}
	...
	specification, err := c.getEntitySpecification(*baseUri, "uipatcleitzc", "DefaultTenant", *identityUri, operationId, false)
	if err != nil {
		return nil, err
	}
...
func (c DataServiceCommand) getEntitySpecification(baseUri url.URL, organization string, tenant string, identityUri url.URL, operationId string, insecure bool) ([]byte, error) {
	// TODO: add caching
	return c.downloadEntitySpecification(baseUri, organization, tenant, identityUri, operationId, insecure)
}

func (c DataServiceCommand) downloadEntitySpecification(baseUri url.URL, organization string, tenant string, identityUri url.URL, operationId string, insecure bool) ([]byte, error) {
	config := c.configProvider.Config("default")
	auth, err := c.executeAuthenticators(config.Auth, baseUri.String(), identityUri, operationId, insecure)
	if err != nil {
		return nil, err
	}
