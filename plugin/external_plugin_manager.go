...
func (m ExternalPluginManager) offline(name string) error {
	...
	directory, err := directories.Offline()
	if err != nil {
		return err
	}
...
func (m ExternalPluginManager) getPlugin(name string, plugin ExternalPluginDefinition) (string, error) {
	...
	tmpPluginsDirectory := m.createTmpFolder(pluginsDirectory, directoryPermissions)
	...
	err = archive.Extract(archivePath, tmpPluginsDirectory, directoryPermissions)
	if err != nil {
