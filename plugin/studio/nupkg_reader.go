...
func (r nupkgReader) ReadNuspec() (*nuspec, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("Could not read package file '%s': %v", r.Path, err)
	}
	defer zip.Close()
	...
	return nil, fmt.Errorf("Could not find nuspec metadata '%s'", r.Path)
}

func (r nupkgReader) readNuspec(source string, file *zip.File) (*nuspec, error) {
	reader, err := file.Open()
	if err != nil {
		return nil, fmt.Errorf("Could not read package file '%s': %v", source, err)
	}
	defer reader.Close()
