...
`
	...
	context := NewContextBuilder().
		WithDefinition("digitizer", definition).
		WithCommandPlugin(plugin_digitizer.StatusCommand{}).
		Build()

	result := runCli([]string{"digitizer", "--help"}, context)
...
`
	...
	result := runCli([]string{"digitizer", "status"}, context)
...
`
	...
	result := runCli([]string{"digitizer", "digitize"}, context)
...
`
	...
	result := runCli([]string{"digitizer", "digitize", "--file", "hello-world"}, context)
...
`
	...
	result := runCli([]string{"digitizer", "digitize", "--file", "file://" + path}, context)
...
`
	...
	result := runCli([]string{"digitizer", "digitize", "--file", "file://" + path, "--debug"}, context)
